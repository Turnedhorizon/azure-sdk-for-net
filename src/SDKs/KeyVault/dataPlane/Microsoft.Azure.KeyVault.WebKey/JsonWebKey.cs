...
namespace Microsoft.Azure.KeyVault.WebKey
{
    ...
    public class JsonWebKey
    {
        ...
        public ECDsa ToECDsa(bool includePrivateParameters = false)
        {
            ...
            var bytes = ecParameters.GetKeyBlob(includePrivateParameters);

            ECDsa ecdsa = null;

            if(includePrivateParameters)
                ecdsa = new ECDsaCng(CngKey.Import(bytes, CngKeyBlobFormat.EccPrivateBlob));
            else
                ecdsa = new ECDsaCng(CngKey.Import(bytes, CngKeyBlobFormat.EccPublicBlob));
